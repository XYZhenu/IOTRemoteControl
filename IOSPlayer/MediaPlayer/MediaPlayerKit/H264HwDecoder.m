...
- (CVPixelBufferRef)decode:(uint8_t *)videoBuffer videoSize:(NSInteger)videoBufferSize dts:(uint32_t)dts pts:(uint32_t)pts
{
    ...
    if (status == kCMBlockBufferNoErr) {
        ...
        CMSampleTimingInfo info = {kCMTimeIndefinite, CMTimeMake(pts, 1000), CMTimeMake(dts, 1000) };
        status = CMSampleBufferCreateReady(kCFAllocatorDefault, blockBuffer, mDecoderFormatDescription , 1, 1, &info, 1, sampleSizeArray, &sampleBuffer);
        ...
        if (pSPS && pPPS && (frameFlag == HWVideoFrameType_I || frameFlag == HWVideoFrameType_P || frameFlag == HWVideoFrameType_B)) {
            ...
            *pNalSize = CFSwapInt32HostToBig(nalSize);
            ...
            if ([self initH264HwDecoder]) {
                pixelBuffer = [self decode:buffer videoSize:videoSize  dts:(uint32_t)dts pts:(uint32_t)pts];
